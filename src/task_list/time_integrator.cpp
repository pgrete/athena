  ...
  : TaskList(pm)
{
  ...
  {using namespace HydroIntegratorTaskNames;
    ...
    if(pm->multilevel==true) { // SMR or AMR
      if(RADIATION_ENABLED){
        AddTimeIntegratorTask(SEND_HYDFLX,CALC_HYDFLX|CALC_RADFLX);
        AddTimeIntegratorTask(RECV_HYDFLX,CALC_HYDFLX|CALC_RADFLX);
      } else {
    ...
    if(RADIATION_ENABLED){
      ...
      AddTimeIntegratorTask(UPDATE_DT,SRCTERM_HYD|SRCTERM_RAD);      
    }else{
    ...
    if(RADIATION_ENABLED){
      AddTimeIntegratorTask(USERWORK,RAD_MOMOPACITY);
    }else{
...
enum TaskStatus TimeIntegratorTaskList::RadFluxes(MeshBlock *pmb, int step)
{
  ...
  if(step == 2) {
    prad->pradintegrator->CalculateFluxes(pmb, phydro->w, prad->ir1, 2);
    return TASK_NEXT;
  }
...
enum TaskStatus TimeIntegratorTaskList::RadIntegrate(MeshBlock *pmb, int step)
{
  ...
  if(integrator == "vl2"){
    if(step == 1){
      ...
      wghts[1] = 0.0;
      wghts[2] = 0.5;
      prad->pradintegrator->FluxDivergence(pmb, prad->ir, prad->ir, wghts, prad->ir1);
      ...
      prad->pradintegrator->FluxDivergence(pmb, prad->ir, prad->ir, wghts, prad->ir);
  ...
  if(integrator == "rk2"){
    if(step == 1){
      ...
      prad->pradintegrator->FluxDivergence(pmb, prad->ir, prad->ir1, wghts, prad->ir);
...
enum TaskStatus TimeIntegratorTaskList::RadSourceTerms(MeshBlock *pmb, int step)
{
  ...
  if(step == 1 && (integrator == "vl2")) dt *= 0.5;
  if(step == 2 && (integrator == "rk2")) dt *= 0.5;
  Real time;
  // *** this must be changed for the RK3 integrator
  if(step == 1) {
    prad->pradintegrator->AddSourceTerms(pmb,dt,ph->u,ph->w,prad->ir1,1);
  } else if(step == 2) {
    ...
    prad->pradintegrator->AddSourceTerms(pmb,dt,ph->u,ph->w,prad->ir,2);
  } else {
    return TASK_FAIL;
  }
