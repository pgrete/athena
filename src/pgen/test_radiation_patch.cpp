...
void MeshBlock::ProblemGenerator(ParameterInput *pin)
{
  ...
  const int Nz = ke - ks + 1;
	...
	const Real nH_patch = pin->GetReal("problem", "nH_patch");
	...
	const int ipxs = pin->GetInteger("problem", "ipxs");
	const int ipys = pin->GetInteger("problem", "ipys");
	const int ipzs = pin->GetInteger("problem", "ipzs");
	const int ipxe = pin->GetInteger("problem", "ipxe");
	const int ipye = pin->GetInteger("problem", "ipye");
	const int ipze = pin->GetInteger("problem", "ipze");
	//set density
  int gis = loc.lx1 * Nx;
  int gjs = loc.lx2 * Ny;
  int gks = loc.lx3 * Nz;
	...
	for (int k=ks; k<=ke; ++k) {
		for (int j=js; j<=je; ++j) {
			for (int i=is; i<=ie; ++i) {
        if (i-is+gis >= ipxs and i-is+gis <= ipxe and
            j-js+gjs >= ipys and j-js+gjs <= ipye and
            k-ks+gks >= ipzs and k-ks+gks <= ipze) {
          phydro->u(IDN, k, j, i) = nH_patch;
        } else {
