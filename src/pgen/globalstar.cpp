  ...
  for (int k=ks; k<=ke; ++k) {
    for (int j=js; j<=je; ++j) {
      for (int i=1; i<=NGHOST; ++i) {
          ...
          Real rho = a(IDN,k,j,is);
          Real tgas = a(IEN,k,j,is)/a(IDN,k,j,is);
          Real ertop = tgas * tgas * tgas * tgas;
          Real sigma = rossopacity(rho,tgas) * rho * lunit * rhounit;
          Real dr= pmb->pcoord->dx1v(is-i);
          Real erbot = ertop + 3.0 * sigma * dr * consFr;
          Real tgasbot = sqrt(sqrt(erbot));
        
          a(IEN,k,j,is-i) = tgasbot * rho;
  ...
  for (int k=ks; k<=ke; ++k) {
    for (int j=js; j<=je; ++j) {
      for (int i=1; i<=NGHOST; ++i) {
        ...
        Real rho = phydro->w(IDN,k,j,is-i);
        Real tgas = phydro->w(IEN,k,j,is-i)/rho;
        ...
        for(int ifr=0; ifr<prad->nfreq; ++ifr){
           ...
           Real jlocal = jtop + 3.0 * dr * kappa * radratio * consFr;
